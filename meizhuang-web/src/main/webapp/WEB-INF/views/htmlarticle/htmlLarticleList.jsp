						...
						<table class="layui-hide" id="test-table-totalRow"
							...
							<script type="text/html" id="test-table-totalRow-toolbarDemo">
								<div class="layui-btn-container">
									<button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="addHtmlLarticle">添加</button>
									<button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="updateHtmlLarticle">修改</button>
									<button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="delHtmlLarticle">删除</button>
								</div>
							</script>
			...
			index : 'lib/index' //主入口模块
				...
				function() {
					...
					table.render({
						...
						defaultToolbar:'',
						autoSort : false,
						url : '/flow/html/getHtmlArticleList',
						method : 'post',
						title : '数据表',
						totalRow : false,
						...
						cols : [ [
							...
							{field:'type', title:'分类', width:150, align:'center',templet:function(d){
								if(d.type==1){
				            		return '跨境物流';
					        	 }else if(d.type==2){
				            		return '跑腿代发';
					        ...
					        {field:'text', title:'内容', width:550, align:'center'},
				    ...
				    table.on('toolbar(test-table-totalRow)', function(obj){
				      ...
				      switch(obj.event){
				        case 'addHtmlLarticle':
				           window.location.href = '/flow/html/addHtmlLarticlePage';
				           
				        break;
				        case 'updateHtmlLarticle':
				        	if(data.length<1){
				        		layer.msg('至少选择一项');
				        		return;
				        		...
				        		var id = data[0].id;
				        		window.location.href = '/flow/html/addHtmlLarticlePage?id='+id;
				        	}
				        	
				        break;
				        case 'delHtmlLarticle':
				        	if(data.length<1){
				        		layer.msg('至少选择一项');
				        		return;
				        	}else if(data.length>1){
				        		...
				        		layer.confirm('请确认选择的删除?', function(index){
									...
									$.post('/flow/html/delHtmlArticle', jsonD,
										function(res) {
											var data = res;
											if (data.success) {
												layer.closeAll();
					...
					//添加提交
					form.on('submit(addHtmlLarticle)', function(data) {
						//layer.alert(JSON.stringify(data.field), {title: '最终的提交信息'});
						if(!$("button[lay-filter='addBankCard']").hasClass("layui-btn-disabled")){
							...
							$.post('/flow/identify/addHtmlLarticleInfo',
									data.field, function(res) {
										setTimeout(function(){
											$("button[lay-filter='addHtmlLarticle']").removeClass("layui-btn-disabled").removeAttr("disabled");
										},3000)
										var data = res;
										if (data.success) {
											layer.closeAll();
					...
					//修改提交
					form.on('submit(updateHtmlLarticle)', function(data) {
						//layer.alert(JSON.stringify(data.field), {title: '最终的提交信息'});
						$.post('/flow/html/updateHtmlArticle',
								data.field, function(res) {
									var data = res;
									if (data.success) {
										layer.closeAll();
			    ...
			    var uploadInst = upload.render({
			      ...
			      ,before: function(obj){
			        ...
			        obj.preview(function(index, file, result){
			          ...
			          $("button[lay-filter='addHtmlLarticle']").addClass("layui-btn-disabled").attr("disabled","disabled");
			        });
			      }
			      ,done: function(res){
			        //如果上传失败
			    ...
			    var uploadInst = upload.render({
			      ...
			      ,before: function(obj){
			        ...
			        obj.preview(function(index, file, result){
			          ...
			          $("button[lay-filter='updateHtmlLarticle']").addClass("layui-btn-disabled").attr("disabled","disabled");
			        });
			      }
			      ,done: function(res){
			        //如果上传失败
			        if(res.code > 0){
			          ...
			          $("button[lay-filter='updateHtmlLarticle']").removeClass("layui-btn-disabled").removeAttr("disabled");
			          return layer.msg('上传失败');
			        }
			        //上传成功
			        $(".qrcodeImg").val(res.data);
...
<!-- 添加模板弹窗 -->
<div id="addHtmlLarticle" class="layui-mask-label"
	...
	<form class="layui-form" action="">
		...
		<div class="layui-form-item" style="margin-top: 20px">
			<div class="layui-input-block">
				<button class="layui-btn" lay-submit lay-filter="addHtmlLarticle">添加</button>
...
<!-- 修改弹窗 -->
<div id="updateHtmlLarticle" class="layui-mask-label"
	style="display: none; padding: 20px;">
	<form class="layui-form" action="">
		<div class="layui-inline layui-form-item">
			<label class="layui-form-label layui-w100"><font
		...
		<div class="layui-form-item" style="margin-top: 20px">
			<div class="layui-input-block">
				<button class="layui-btn" lay-submit lay-filter="updateHtmlLarticle">修改</button>
				<button type="reset" class="layui-btn layui-btn-primary closeBtn">取消</button>
			</div>
		</div>
		<input type="hidden" name="id" class="id" />
